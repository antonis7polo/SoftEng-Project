...
jest.mock('../../src/apiClient', () => ({
    uploadTitleCrew: jest.fn(),
}));

const fs = require('fs');
const { uploadTitleCrew } = require('../../src/apiClient');
const { Command } = require('commander');
const newCrewCommand = require('../../src/commands/newcrew');

describe('newCrewCommand', () => {
    let program;
    ...
    it('successfully uploads a file', async () => {
      const mockFilename = '/path/to/crew.tsv';
      fs.existsSync.mockReturnValue(true);
      const mockResult = { message: 'Upload successful' };
      uploadTitleCrew.mockResolvedValue(mockResult);
  
      console.log = jest.fn(); // Mock console.log to capture output
  
      await program.parseAsync(['node', 'test', 'newcrew', '--filename', mockFilename]);
  
      expect(fs.existsSync).toHaveBeenCalledWith(mockFilename);
      expect(uploadTitleCrew).toHaveBeenCalledWith(mockFilename);
    ...
    it('handles error when upload fails', async () => {
      ...
      uploadTitleCrew.mockRejectedValue(new Error(errorMessage));
